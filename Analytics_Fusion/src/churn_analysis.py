...
def churn_analysis_compute_churn_scores(data):
    ...
    # Calculate the recency of the most recent purchase for each client
    recency = data.groupby('Client_ID')['Recency'].min()

    # Create a DataFrame containing the frequency and recency for each client
    churn_data = pd.DataFrame({'Frequency': frequency, 'Recency': recency})

    # Transform frequency to represent churn likelihood (higher frequency => lower likelihood)
    churn_data['Frequency'] = 1 / (1 + churn_data['Frequency'])

    # Normalize recency to range [0, 1]
    churn_data['Recency'] = churn_data['Recency'] / churn_data['Recency'].max()

    # Compute churn score as the average of frequency and recency scores
...
def churn_analysis_apply_kmeans_clustering(churn_data, n_clusters=4):
    # Fit K-Means clustering to churn data using the specified number of clusters
    ...
    # Compute distances from each data point to its respective cluster centroid
    distances = np.linalg.norm(churn_data[['Churn_Score']].values - cluster_centers[churn_data['Cluster']], axis=1)
    # Compute confidence scores based on the distances
    ...
    # Return the churn data with assigned clusters, confidence scores, and the K-Means model
...
def churn_analysis_create_pie_chart(churn_data, kmeans):
    # Count the number of clients in each churn cluster
    cluster_counts = churn_data['Cluster_Name'].value_counts()

    # Compute the average churn score and confidence for each cluster
    ...
    # Prepare labels for each cluster with additional information
    labels = [f'{name}\n{count} clients\nAvg Churn: {churn:.2f}\nConfidence: {conf:.2f}'
    ...
    # Add title and legend to the pie chart
    ax.set_title('Client Churn Clusters')
    ax.legend(['Safe: Low Churn Score', 'Low Risk: Relatively Stable', 'Moderate Risk: Showing Signs of Churn', 'High Risk: High Churn Likelihood'],
